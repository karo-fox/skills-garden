...
class TestModels(TestCase):
    ...
    def test_field_admin_topic_link(self):
        self.assertEqual(self.field1.admin_topic_filter_link(
        ), '<a href="/admin/garden/topic/?field__id__exact=1">topics</a>')
    ...
    def test_topic_get_absolute_url(self):
        self.assertEqual(self.topic1.get_absolute_url(),
                         '/garden/fields/1/topics/1/')

    def test_topic_admin_link(self):
        self.assertEqual(self.topic1.admin_field_link(
        ), '<a href="/admin/garden/field/1/change/">Test field</a>')
...
class TestViews(APITestCase):
    ...
    def test_fields_update_view(self):
        ...
        response = self.client.put(
            reverse('garden:field-detail', kwargs={'pk': 1}), data)

        self.assertEqual(response.status_code, 200)

    def test_fields_delete_view(self):
        response = self.client.delete(
            reverse('garden:field-detail', kwargs={'pk': 1}))

        self.assertEqual(response.status_code, 204)

    def test_topics_get_view(self):
        response = self.client.get(
            reverse('garden:topic-list', kwargs={'field_pk': 1}))
    ...
    def test_topics_post_view(self):
        ...
        response = self.client.post(
            reverse('garden:topic-list', kwargs={'field_pk': 1}), data)
    ...
    def test_topics_update_view(self):
        ...
        response = self.client.put(
            reverse('garden:topic-detail', kwargs={'pk': 1, 'field_pk': 1}), data)

        self.assertEqual(response.status_code, 200)

    def test_topics_delete_view(self):
        response = self.client.delete(
            reverse('garden:topic-detail', kwargs={'pk': 1, 'field_pk': 1}))
